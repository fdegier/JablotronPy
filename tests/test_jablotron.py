...
class TestJablotron(TestCase):
    ...
    def test_get_programmable_gates(self):
        ...
        sections = self.jablotron.get_programmable_gates(
            service_id=services[1]["service-id"]
        )
    ...
    def test_control_programmable_gate(self):
        ...
        pg_id = self.jablotron.get_programmable_gates(service_id=service_id)["states"][
            1
        ]["cloud-component-id"]
        data = self.jablotron.control_programmable_gate(
            service_id=service_id, component_id=pg_id, on=False
        )
